...
export type ConfirmStoredParticipantAccessInput = {
  participantId: string;
  birthYear: string;
};

export type ConfirmStoredParticipantAccessResult = {
  participantId: string;
  eventSlug: string;
  firstName?: string;
  lastName?: string;
...
export async function confirmStoredParticipantAccess({
  participantId,
  birthYear,
}: ConfirmStoredParticipantAccessInput): Promise<ConfirmStoredParticipantAccessResult | null> {
  ...
  const participantRes = await supabase
    .from("participants")
    .select(
      "id, event_slug, first_name, last_name, birth_year, email, recovery_token, default_gedcom_person_id",
    )
  ...
  return {
    ...
    defaultGedcomPersonId: participant.default_gedcom_person_id ?? undefined
  };
}