// src/features/player/experiences/collect/hooks/useCollectActivityPlayer.ts
...
function getImmediateQuestionPoints(
    question: ActivityQuestionDefinition
): number {
...
export function useCollectActivityPlayer(
    ...
    const scoringEnabled = activity.scoring.kind === "enabled";
    ...
    }, [isComplete, pendingReviewScore, score, sessionId]);
    ...
    async function submitCurrent() {
        ...
        const immediateDelta =
            !alreadySubmitted && scoringEnabled
                ? getImmediateQuestionPoints(currentQuestion)
                : 0;
        const pendingDelta =
            !alreadySubmitted && scoringEnabled
                ? getPendingReviewPoints(currentQuestion)
        ...
        const nextScore = score + immediateDelta;
        ...
        try {
            ...
            window.setTimeout(() => {
                void goToResolvedNextQuestion().finally(() => {
