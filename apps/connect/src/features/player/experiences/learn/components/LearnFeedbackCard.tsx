...
function getTone(params: {
    ...
    isRetry: boolean;
    isSkipped: boolean;
    isNeutralSubmission: boolean;
}): Tone {
    const { isSuccess, isManualReview, isRetry, isSkipped, isNeutralSubmission } =
        params;
...
export function LearnFeedbackCard({
    ...
    const Icon = isSkipped
        ? SkipForward
        : isManualReview || isNeutralSubmission
          ? Info
          : isSuccess
            ? CheckCircle2
            : isRetry
    ...
    const showInfoCard =
        !isRetry &&
        !isSkipped &&
        (canShowExpectedAnswer || canShowExplanation || Boolean(result.setSummary));
    ...
    const displayedDelta = result.cumulativeScoreDelta ?? result.scoreDelta;
    const deltaPrefix = displayedDelta > 0 ? "+" : "";
    ...
    const showSetSummary = Boolean(result.setSummary);

    return (
        <div className="fixed inset-0 z-[100] overflow-y-auto bg-white">
            <div className="flex min-h-full flex-col justify-start px-4 py-6 pb-[calc(env(safe-area-inset-bottom)+16px)] sm:justify-center">
                            ...
                            {showInfoCard ? (
                                ...
                                >
                                    {showSetSummary ? (
                                        <div className="rounded-2xl border border-slate-200 bg-white p-3">
                                            <div className="text-xs font-black uppercase tracking-wide text-slate-600">
                                                Détail de ta sélection
                                            </div>

                                            <div className="mt-3 grid gap-2 text-sm font-bold text-slate-700">
                                                <div>
                                                    Bonnes réponses sélectionnées :{" "}
                                                    <span className="text-slate-900">
                                                        {result.setSummary?.correctSelected} / {result.setSummary?.totalCorrect}
                                                    </span>
                                                </div>

                                                <div>
                                                    Réponses incorrectes cochées :{" "}
                                                    <span
                                                        className={
                                                            result.setSummary?.incorrectSelected
                                                                ? "text-rose-700"
                                                                : "text-emerald-700"
                                                        }
                                                    >
                                                        {result.setSummary?.incorrectSelected ?? 0}
                                                    </span>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    {canShowExpectedAnswer ? (
                                        <div className={showSetSummary ? "mt-4" : ""}>
                                            <div className="text-xs font-black uppercase tracking-wide text-slate-600">
                                    ...
                                    {canShowExplanation && result.explanation ? (
                                        <SimpleFormattedText
                                            text={result.explanation}
                                            className={[
                                                "text-[15px] leading-6 font-medium text-slate-800",
                                                showSetSummary || canShowExpectedAnswer ? "mt-4" : "",
                                            ].join(" ")}
                                        />
                                    ) : null}
                                </div>
