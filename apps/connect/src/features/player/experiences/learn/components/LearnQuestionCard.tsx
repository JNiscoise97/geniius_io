  ...
  return (
    <div className="mt-4 grid gap-4">
      {media.map((item, index) => {
        const key = `${item.kind}-${item.src}-${index}`;
        ...
        if (item.kind === "image") {
          return (
            ...
            >
              <SmartImage src={item.src} alt={item.alt ?? "Illustration"} />
              {item.caption ? (
                <figcaption className="border-t border-slate-200 bg-white px-4 py-3 text-xs font-bold text-slate-600">
                  {item.caption}
                </figcaption>
              ) : null}
            </figure>
        ...
        if (item.kind === "audio") {
          return (
            ...
            >
              ...
              {item.caption ? (
                <figcaption className="mt-3 text-xs font-bold text-slate-600">
                  {item.caption}
                </figcaption>
              ) : null}
            </figure>
        ...
        if (item.kind === "video") {
          return (
            <figure
              ...
              className="overflow-hidden rounded-2xl border border-slate-200 bg-slate-50"
            >
              <div className="flex items-center gap-2 border-b border-slate-200 bg-white px-4 py-3 text-sm font-black text-slate-900">
                <PlayCircle size={16} />
                Vidéo
              </div>
        ...
        return (
          ...
          >
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="mt-3 inline-flex rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-black text-slate-700 transition hover:border-slate-300"
            >
              Ouvrir le document
            </a>
...
function TextBlock({
  ...
  question: Extract<ActivityQuestionDefinition, { type: "short" | "long" | "fill" }>;
  answer: LearnAnswerValue;
  disabled: boolean;
  onAnswerChange: (value: LearnAnswerValue) => void;
  label?: string;
    ...
    <div className="mt-4 rounded-3xl border border-indigo-100 bg-indigo-50 p-4">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0">
          ...
          <SimpleFormattedText
            text={question.bodyMarkdown}
            className="mt-2 text-[15px] font-medium leading-6 text-slate-800"
          />
    ...
    <div className="mt-4 grid gap-4">
      <div className="rounded-2xl border border-indigo-100 bg-indigo-50 p-4">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            ...
            <div className="mt-2 text-xs font-bold text-slate-600">
              Cette réponse sera relue par l’organisateur. Les points seront
              ajoutés après modération.
            </div>
      ...
      {previewUrl ? (
        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-slate-50">
          <SmartImage src={previewUrl} alt="Aperçu de la photo" />
        </div>
      ) : null}
...
export function LearnQuestionCard({
  ...
  return (
    <div className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <QuestionMediaRenderer media={question.media} />
