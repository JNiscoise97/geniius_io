...
for (const [code, n] of [...counts.entries()].sort((a,b)=>b[1]-a[1])) console.log(' ', code, n)

console.log('\n=== NEW HYPOTHEQUES PREDICATES CHECK ===')
const newPreds = ['seller','buyer','sale_price','deceased','heir','usufructuary','area','boundary','registration_date','registration_place','registration_volume','registration_folio','registration_number','marital_regime']
for (const p of newPreds) console.log(' ', p, ':', counts.get(p) ?? 0)

console.log('\n=== SELLER/BUYER/DECEASED/HEIR/USUFRUCTUARY/AREA/BOUNDARY DETAIL ===')
for (const a of assertions) {
  const code = a.ref_assertion_predicates?.code
  if (['seller','buyer','deceased','heir','usufructuary','area','boundary'].includes(code)) console.log(fmt(a))
}

console.log('\n=== JUSTIN / CASIMIR CHARBONNE CHECK (fusion bug) ===')
...
console.log('\n=== PLACES ===')
for (const e of entities) if (e.entity_type === 'place') console.log(' ', e.local_key, '-', e.label)
