...
export default function NotaireRegistreLayout() {
  const { id } = useParams();
  const { notaireId } = useParams();
  const [registre, setRegistre] = useState<NotaireRegistre | null>(null);
  const [notaire, setNotaire] = useState<Notaire | null>(null);
  const [isLoading, setIsLoading] = useState(false);
  const fetchRegistre = useNotaireRegistreStore((s) => s.fetchRegistre);
  const fetchNotaire = useNotaireStore((s) => s.fetchOne);

  useEffect(() => {
    if (!id || !notaireId) return;

    setIsLoading(true);

    Promise.all([fetchRegistre(id), fetchNotaire(notaireId)])
      .then(([reg, not]) => {
        setRegistre(reg ?? null);
        setNotaire(not ?? null);
      })
      .catch((err) => {
        console.error('Erreur lors du chargement du notaire ou du bureau', err);
        setRegistre(null);
        setNotaire(null);
      })
  ...
  const handleNewActe = (newActe: ActeBase) => {
...
function DataTableActes({
  ...
  const columns: ColumnDef<any>[] = [
    ...
    },
    {
      key: 'date',
      label: 'Date',
      render: (row) => (row.__vide ? <span className='text-gray-400 italic'>—</span> : formatDateToNumericFrench(row.date)),
      columnWidth: '8%',
    },
    {
      key: 'source',
    ...
    },
    {
      ...
      render: (row) =>
        row.__vide ? (
          ...
          <Link to={`/ac-acte/${row.id}`} className='font-medium'>
            {row.label}
          </Link>
        ),
        columnWidth: '60%',
