...
function ReferencerDialog({
  ...
  docs: Document[]
  onClose: () => void
  onCreated: (data: ReferencerFormData) => Promise<void>
}) {
  ...
  const results = query.trim().length > 1
    ? docs.filter(d =>
        ...
        d.cote.toLowerCase().includes(query.toLowerCase())
      )
    : []
  ...
  async function handleCreate() {
    ...
    try {
      await onCreated({
        ...
        parent_ud_id: sourceId || null,
        couverture_label: date || null,
        cote: cote || null,
        note: note || null,
      })
    } finally {
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          <h2 className="text-sm font-semibold text-gray-900">
            {step === 'search' ? 'Référencer un document' : 'Nouveau document'}
          </h2>
          ...
          <div className="p-5 space-y-4">
            <div className="relative">
              ...
              <input
                ...
                placeholder="Chercher par titre, cote…"
                className="w-full pl-9 pr-3 py-2.5 text-sm border border-gray-200 rounded-xl bg-gray-50 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white"
              />
            </div>
            ...
            {query.trim().length > 1 && (
              <div className="space-y-1">
                {results.length > 0 ? (
                  <>
                    <p className="text-xs text-gray-400 pb-1">{results.length} document{results.length > 1 ? 's' : ''} trouvé{results.length > 1 ? 's' : ''}</p>
                    {results.map(doc => {
                      const statut = STATUT_DOC_CONFIG[doc.statut]
                      return (
                        <div key={doc.id} className="flex items-center gap-3 p-3 rounded-xl border border-gray-100 bg-gray-50 hover:border-indigo-200 hover:bg-indigo-50/30 cursor-pointer transition-colors group">
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-gray-900 truncate group-hover:text-indigo-700">{doc.titre}</p>
                            <p className="text-xs text-gray-400 font-mono">{doc.cote}</p>
                          </div>
                          <span className={`text-xs font-medium rounded-full border px-2 py-0.5 flex items-center gap-1 shrink-0 ${statut.color}`}>
                  ...
                  <p className="text-xs text-gray-400 py-2">Aucun document trouvé pour cette recherche.</p>
                )}
              </div>
            )}
            ...
            <div className="pt-2 border-t border-gray-100">
              <button
                ...
                className="w-full flex items-center justify-center gap-2 rounded-xl border-2 border-dashed border-gray-200 py-3 text-sm font-medium text-gray-500 hover:border-indigo-300 hover:text-indigo-600 transition-colors"
              >
                <Plus className="w-4 h-4" />
                Référencer un nouveau document
              </button>
            </div>
          </div>
        )}
        ...
        {step === 'form' && (
          <div className="p-5 space-y-4">
            <p className="text-xs text-gray-500">
              Référencez rapidement. Vous pourrez compléter la description plus tard depuis l'onglet <strong>En attente</strong>.
            </p>
            ...
            <div>
              ...
              <select
                ...
                className="w-full text-sm border border-gray-200 rounded-xl px-3 py-2.5 bg-white text-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-500"
              >
                <option value="">Sélectionner une source…</option>
                {sources.filter(s => s.statut !== 'a_qualifier').map(s => (
                  <option key={s.id} value={s.id}>{s.nom}</option>
                ))}
                <option value="">— Source inconnue (à rattacher)</option>
              </select>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  placeholder="Titre approximatif"
                  className="w-full text-sm border border-gray-200 rounded-xl px-3 py-2.5 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1.5">Cote / référence</label>
                <input
                  value={cote}
                  onChange={e => setCote(e.target.value)}
                  placeholder="ex. 3Q 4501"
                  className="w-full text-sm border border-gray-200 rounded-xl px-3 py-2.5 placeholder-gray-400 font-mono focus:outline-none focus:ring-2 focus:ring-indigo-500"
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1.5">Date approximative</label>
                <input
                  value={date}
                  onChange={e => setDate(e.target.value)}
                  placeholder="ex. 1889 ou 1880–1900"
                  className="w-full text-sm border border-gray-200 rounded-xl px-3 py-2.5 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1.5">Note rapide</label>
                <input
                  value={note}
                  onChange={e => setNote(e.target.value)}
                  placeholder="Contexte de découverte…"
                  className="w-full text-sm border border-gray-200 rounded-xl px-3 py-2.5 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                />
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 pt-2">
              ...
              <button
                ...
                disabled={submitting}
                className="flex-1 rounded-xl bg-indigo-600 py-2.5 text-sm font-medium text-white hover:bg-indigo-700 transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
              >
...
export function SourcesCorpusPage() {
  ...
  const { sources, docs, corpus, loading, error, refetch, addDoc } = usePatrimoine()
  ...
  async function handleDocCreated(data: ReferencerFormData) {
    await addDoc({
      titre: data.titre,
      parent_ud_id: data.parent_ud_id,
      couverture_label: data.couverture_label,
      cote: data.cote,
      note: data.note,
    })
    setShowReferencer(false)
    setTab('En attente')
  }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="flex items-start justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() => setShowReferencer(true)}
            ...
            <button onClick={() => void navigate('/mock/sources-identifier')}
      ...
      {showReferencer && (
        <ReferencerDialog
          sources={sources}
          docs={docs}
          onClose={() => setShowReferencer(false)}
