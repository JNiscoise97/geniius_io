-- Vue agrégée pour l'onglet "Sources" (parent_ud_id IS NULL) : unité
-- documentaire + son meilleur exemplaire + statistiques de ses documents
-- enfants. Portée depuis public.v_sources, adaptée aux tables/colonnes
-- renommées de ce schéma :
--   - ref_unites_documentaires -> rebond.unites_documentaires
--   - ref_exemplaires -> rebond.exemplaires
--   - u.statut/u.workflow_statut -> u.statut_source/u.statut_document,
--     ré-aliasés en sortie vers statut/workflow_statut pour que la vue
--     garde exactement la même forme de sortie (VSourceRow côté app,
--     aucun changement requis dans patrimoine.service.ts au-delà du client).
-- ref_natures n'est pas encore migrée : jointure cross-schema vers public.
...
comment on view rebond.v_sources is
  'Sources (unités documentaires racines) avec meilleur exemplaire et stats. Voir apps/rebond/supabase/schema-docs/v_sources.md';
