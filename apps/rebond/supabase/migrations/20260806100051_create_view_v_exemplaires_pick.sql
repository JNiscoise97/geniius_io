-- Vue de sélection d'un exemplaire enrichie (dépôt, institution, série,
-- bureaux et types d'actes associés, meilleure URL). Portée depuis
-- public.v_exemplaires_pick, adaptée aux tables renommées :
--   - ref_unites_documentaires -> rebond.unites_documentaires
--   - ref_exemplaires -> rebond.exemplaires
--   - ref_ec_type_acte -> rebond.ref_etat_civil_type_acte
-- ref_natures et ref_supports pas encore migrées : cross-schema vers public.
-- ref_unites_documentaires_bureaux et ref_unites_documentaires_types_actes
-- (pivots) n'ont pas été fournies dans ce lot : cross-schema vers public
-- également — une jointure SQL classique (contrairement à un embed
-- PostgREST) fonctionne très bien entre schémas, pas besoin de les migrer
...
with
  ...
  agg_bureaux as (
    ...
    from public.ref_unites_documentaires_bureaux rub
  ...
  agg_types_actes as (
    ...
    from public.ref_unites_documentaires_types_actes ruta
...
comment on view rebond.v_exemplaires_pick is
  'Sélection enrichie d''un exemplaire (dépôt, institution, série, bureaux, types d''acte). Voir apps/rebond/supabase/schema-docs/v_exemplaires_pick.md';
