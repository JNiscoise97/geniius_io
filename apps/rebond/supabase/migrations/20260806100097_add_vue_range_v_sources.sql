-- Expose exemplaires.localisation_interne (la plage de vues du meilleur
-- exemplaire, ex. "1-250" pour un registre numérisé) sur rebond.v_sources,
-- sous le nom vue_range. Nécessaire pour situer une table/un acte dans
-- l'étendue totale du registre associé côté carte "Documents à décrire"
-- (même logique que le récapitulatif du wizard de référencement).
-- CREATE OR REPLACE VIEW : ajout d'une colonne en fin de liste, ne casse
-- aucun select explicite existant.
...
select
  ...
  be.localisation_interne as vue_range
from rebond.unites_documentaires u
